...
APP_ID = "biblioteca.municipal.desktop.v1"


def ensure_icon_file() -> Path:
    if ICON_PATH.exists():
        return ICON_PATH
    temp_icon = Path(gettempdir()) / "biblioteca_app_icon.ico"
    try:
        from scripts.generate_icon import build_ico
...
class BibliotecaDesktopApp(tk.Tk):
    ...
    def _build_inicio(self):
        ...
        for col, text, width in [("id", "ID", 70), ("libro", "Libro", 320), ("socio", "Socio", 250), ("limite", "Fecha límite", 120)]:
            self.atrasados_tree.heading(col, text=text, anchor="center")
            self.atrasados_tree.column(col, width=width, anchor="center")
    ...
    def _build_libros(self):
        ...
        for col, text, w in [("id", "ID", 70), ("titulo", "Título", 250), ("autor", "Autor", 190), ("categoria", "Categoría", 140), ("isbn", "ISBN", 130), ("disp", "Disponibles", 100)]:
            self.libros_tree.heading(col, text=text, anchor="center")
            self.libros_tree.column(col, width=w, anchor="center")
    ...
    def _build_socios(self):
        ...
        for col, text, w in [("id", "ID", 70), ("nombre", "Nombre", 200), ("email", "Email", 250), ("telefono", "Teléfono", 130), ("direccion", "Dirección", 250)]:
            self.socios_tree.heading(col, text=text, anchor="center")
            self.socios_tree.column(col, width=w, anchor="center")
    ...
    def _build_prestamos(self):
        ...
        for col, text, w in [("id", "ID", 60), ("libro", "Libro", 220), ("socio", "Socio", 190), ("inicio", "Inicio", 110), ("limite", "Límite", 110), ("estado", "Estado", 90)]:
            self.prestamos_tree.heading(col, text=text, anchor="center")
            self.prestamos_tree.column(col, width=w, anchor="center")
